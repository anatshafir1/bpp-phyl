...
std::vector<size_t> JointMLAncestralReconstruction::getAncestralStatesForNode(uint nodeId) const{
    ...
    if (nodeId == tree_->getNodeIndex(tree_->getRoot())){
        ...
        for (size_t i = 0; i < nbDistinctSites_; i++){
            ExtendedFloat rootStateForCurrSite_i = rootReconstruction(i);
            size_t rootStateForCurrSite = (size_t)ExtendedFloat::convert(rootStateForCurrSite_i);
        ...
        for (size_t i = 0; i < nbDistinctSites_; i++){
            ...
            ExtendedFloat nodeReconstruction_father_i = nodeReconstruction(fatherState, i);
            size_t currNodeState = (size_t)(ExtendedFloat::convert(nodeReconstruction_father_i));
