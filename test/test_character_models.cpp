...
double getTransitionProb(size_t i, size_t j, double d, RowMatrix<double>& rates, const vector<double>& freqs)
{
...
void test_binary_model_settings(CharacterSubstitutionModel& model, RatesParameterizationScope ratesParameterizationScope, RowMatrix<double>& rates, const vector<double>& freqs)
{
    // test rate matrix
...
void test_multistate_models(const IntegerAlphabet* alpha){
    std::cout << "*********************************" << std::endl;
    std::cout << " MultiState models " << std::endl;
    std::cout << "*********************************" << std::endl;
    cout << "case 1.8: multistate models" << std::endl;
    shared_ptr<IntegerFrequencySet> freqs;
    RowMatrix<double> rateVals(3, 3);
    vector<double> freqVals(3);
    freqVals[0] = 0.2;
    freqVals[1] = 0.5;
    freqVals[2] = 0.3;
    for (size_t i=0; i<3; ++i)
    {
        ...
        {
            rateVals(i,j) = ((i == j) ?  -2: 1);
        }
    }
    rateVals(0, 1) = 2.,
    rateVals(0, 2) = 3.;
    rateVals(0, 0) = -rateVals(0, 1)-rateVals(0, 2);
    rateVals(1, 0) = 4.;
    rateVals(1, 2) = 6.;
    rateVals(1, 1) = -rateVals(1, 0)-rateVals(1, 2);
    rateVals(2, 0) = 6.;
    rateVals(2, 1) = 2.;
    rateVals(2, 2) = -rateVals(2, 0)-rateVals(2, 1);
    ...
    ratePerPairModel.setParameterValue("rate_1_2", rateVals(1, 2));
    ratePerPairModel.setParameterValue("rate_2_1", rateVals(2, 1));
    ratePerPairModel.setParameterValue("rate_0_2", rateVals(0, 2));
    ratePerPairModel.setParameterValue("rate_2_0", rateVals(2, 0));
    ratePerPairModel.setFrequencySet(*freqs);
    auto& Q6 = ratePerPairModel.getGenerator();
    MatrixTools::printForR(Q6,"Q",std::cout);
    VectorTools::printForR(ratePerPairModel.getFrequencies(),"freq",std::cout);
    vector<double> eigenvalues = ratePerPairModel.getEigenValues();
    std::cout << "Eigen values: "<< std::endl;
    for (size_t i = 0; i < eigenvalues.size(); i++){
        std::cout << "lambda" << i << " " << eigenvalues[i] << std::endl;
    }
    auto params = ratePerPairModel.getParameters();
    for (size_t i = 0; i < params.size(); i++){
        std::cout << params[i].getName() << " = " << params[i].getValue() << std::endl;

    }
    ratePerPairModel.setParameterValue("Full.theta1", 0.4);
    VectorTools::printForR(ratePerPairModel.getFrequencies(),"freq",std::cout);
...
void test_binary_models(const IntegerAlphabet* alpha)
{
    ...
    auto& Q0 = noRatenoFreqsModel.getGenerator();
    MatrixTools::printForR(Q0,"Q",std::cout);
    VectorTools::printForR(noRatenoFreqsModel.getFrequencies(),"freq",std::cout);

    test_binary_model_settings(noRatenoFreqsModel, RatesParameterizationScope::noRate, rateVals, noRatenoFreqsModel.getFrequencies());
    ...
    auto& Q1 = noRateModel.getGenerator();
    MatrixTools::printForR(Q1,"Q",std::cout);
    VectorTools::printForR(noRateModel.getFrequencies(),"freq",std::cout);

    test_binary_model_settings(noRateModel, RatesParameterizationScope::singleRate, rateVals, noRateModel.getFrequencies());
    ...
    MatrixTools::printForR(Q2,"Q",std::cout);
    VectorTools::printForR(oneRateModel.getFrequencies(),"freq",std::cout);
    test_binary_model_settings(dynamic_cast<CharacterSubstitutionModel&>(oneRateModel), RatesParameterizationScope::singleRate, rateVals, oneRateModel.getFrequencies());
    ...
    VectorTools::printForR(ratePerEntryModel.getFrequencies(),"freq",std::cout);
    std::cout << "\n";
    ...
    auto& Q3 = ratePerEntryModel.getGenerator();
    MatrixTools::printForR(Q3,"Q",std::cout);
    VectorTools::printForR(ratePerEntryModel.getFrequencies(),"freq",std::cout);
    test_binary_model_settings(dynamic_cast<CharacterSubstitutionModel&>(ratePerEntryModel), RatesParameterizationScope::ratePerEntry, rateVals, ratePerEntryModel.getFrequencies());
    ...
    auto& Q4 = ratePerExitModel.getGenerator();
    MatrixTools::printForR(Q4,"Q",std::cout);
    VectorTools::printForR(ratePerExitModel.getFrequencies(),"freq",std::cout);

    test_binary_model_settings(dynamic_cast<CharacterSubstitutionModel&>(ratePerExitModel), RatesParameterizationScope::ratePerExit, rateVals, ratePerExitModel.getFrequencies());
    ...
    auto& Q5 = ratePerPairSymModel.getGenerator();
    MatrixTools::printForR(Q5,"Q",std::cout);
    VectorTools::printForR(ratePerPairSymModel.getFrequencies(),"freq",std::cout);
    test_binary_model_settings(dynamic_cast<CharacterSubstitutionModel&>(ratePerPairSymModel), RatesParameterizationScope::ratePerPairSym, rateVals, ratePerPairSymModel.getFrequencies());
    ...
    MatrixTools::printForR(Q6,"Q",std::cout);
    VectorTools::printForR(ratePerPairModel.getFrequencies(),"freq",std::cout);
    test_binary_model_settings(dynamic_cast<CharacterSubstitutionModel&>(ratePerPairModel), RatesParameterizationScope::ratePerPair, rateVals, ratePerPairModel.getFrequencies());
...
int main() 
{
    try
    {
        ...
        const IntegerAlphabet* int_alphabet = new IntegerAlphabet(numOfStates-1);
        test_binary_models(int_alphabet);
        const IntegerAlphabet* alpha = new IntegerAlphabet(2);
        test_multistate_models(alpha);
    }
        catch (exception & e)
    {
        cout << e.what() << endl;
